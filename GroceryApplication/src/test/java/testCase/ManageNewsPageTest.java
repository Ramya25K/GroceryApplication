...
public class ManageNewsPageTest extends BaseClass {
  ...
  public void addNewsInformation() throws IOException {
		...
		lp.loginUsingExcel();
		mn.clickOnManageNewsTab();
		mn.clickOnNewManageNewsButton();
		String newsData = mn.enterNewsInformation("This is a Grocery Application currently undergoing testing",false);//false- add news
		boolean actualText = mn.getAlertText();
		boolean expectedText = true;  // expected result should be true
		assertEquals(actualText, expectedText, "Error in saving"); // if the save doesnt work, it will print "Error in Saving".
  }
  ...
  public void editNewsInformation() throws IOException {
		...
		String editedNewsData = mn.enterNewsInformation("This is a Grocery Application currently undergoing testing",true);//true - edit news
